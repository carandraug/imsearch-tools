...
print(
    "Retrieved %d result URLs in %f seconds"
    % (len(results), (time.time() - t))
)
...
output_fns = imgetter.process_urls(
    results, outdir, test_callback, process_images=False
)
print(
    "Downloaded %d results in %f seconds"
    % (len(output_fns), (time.time() - t))
)
