...
def callback_func(out_dict, extra_prms=None):
    ...
    log.info(
        "VISOR CATEGORY: Prepared request to VISOR backend (%s): %s",
        debug_cb_id,
        request,
    )
    ...
    while term_idx < 0:
        try:
            log.debug(
                "VISOR CATEGORY: Receiving response chunk (%s)...", debug_cb_id
            )
            rep_chunk = sock.recv(1024)
            rep_chunk = rep_chunk.decode()
            if not rep_chunk:
                log.error(
                    "VISOR CATEGORY: Connection closed! (%s)", debug_cb_id
                )
                sock.close()
    ...
    log.debug(
        "VISOR CATEGORY: Received response (%s): %s", debug_cb_id, response
    )
    sock.close()
    log.debug("VISOR CATEGORY: Closed socket (%s)", debug_cb_id)
    ...
    if "zmq_impath_return_ch" in extra_prms:
        log.info(
            "VISOR CATEGORY: Returning image URL on ZMQ channel: %s",
            extra_prms["zmq_impath_return_ch"],
        )
        created_sock = True
        try:
        ...
        log.info(
            "VISOR CATEGORY: Not returning image URL over ZMQ channel (not specified)"
        )
