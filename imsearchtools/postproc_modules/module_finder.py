...
def get_module_callback(module_name):
    try:
        module = importlib.import_module(
            "." + module_name, "imsearchtools.postproc_modules"
        )
        callback_func = module.callback_func
    except ImportError as err:
        avail_modules = get_module_list()
        raise ImportError(
            "Could not find postproc module with name: '%s' (available modules are %s): %s"
            % (module_name, avail_modules, str(err))
        )
    except AttributeError:
        raise AttributeError(
            "Postproc module with name '%s' did not contain required 'callback_func(out_dict)' function"
            % module_name
        )
...
def get_module_list():
    ...
    modnames = [
        ...
        and os.path.splitext(modname)[0]
