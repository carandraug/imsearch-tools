...
def combine_results_pages(
    results_arr, generator_name_arr, output_filename, show_in_browser=True
):
    ...
    for res_idx in range(len(results_arr)):
        ...
        result_frame_header_code = result_frame_header_code.replace(
        ...
        frame_code = frame_code.replace(
            "<!-- RESULT_FRAME_HEADER -->", result_frame_header_code
        )
        frame_code = frame_code.replace("<!-- RESULTS -->", result_grid_code)

        frames_code += frame_code

    combined_page = combined_page.replace(
        "<!-- RESULT_FRAMES -->", frames_code
    )
