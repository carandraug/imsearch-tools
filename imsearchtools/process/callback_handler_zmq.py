...
ZMQ_TASK_COUNT_DEC_CH = (
    "ipc:///tmp/zmq_imsearchtools_tcdec_ch_" + pipe_name_hash
)
ZMQ_TASK_RESULT_CH = "ipc:///tmp/zmq_imsearchtools_result_ch_" + pipe_name_hash
ZMQ_WORKER_CONTROL_CH = (
    "ipc:///tmp/zmq_imsearchtools_control_ch_" + pipe_name_hash
)
...
class CallbackHandler:
    ...
    def __init__(self, worker_func, task_count, worker_count=-1):
        ...
        if PROC_TYPE == ProcType.gipc:
            self.result_manager = gipc.start_process(
                target=result_manager, args=(task_count,)
            )
        elif PROC_TYPE == ProcType.multiprocessing:
...
class CallbackTaskRunner:
    ...
    def run(self, *args, **kwargs):
        ...
        params = dict(args=args, kwargs=kwargs)
        if DEBUG_CB:
...
class CallbackTaskWorkers:
    ...
    def __init__(self, worker_func, worker_count):
        ...
        for wrk_num in range(worker_count):
            if PROC_TYPE == ProcType.gipc:
                self.workers[wrk_num] = gipc.start_process(
                    target=self._callback_worker, args=(wrk_num, worker_func)
                )
            elif PROC_TYPE == ProcType.multiprocessing:
                self.workers[wrk_num] = Process(
                    target=self._callback_worker, args=(wrk_num, worker_func)
                )
                self.workers[wrk_num].start()
            elif PROC_TYPE == ProcType.greenlet:
                self.workers[wrk_num] = gevent.spawn(
                    self._callback_worker, wrk_num, worker_func
                )
    ...
    while True:
        ...
        if socks.get(result_receiver) == zmq.POLLIN:
            ...
            if result_message == ZMQ_RESULT_DONE:
                num_tasks -= 1
                log.debug(
                    "Completed post-computation, remaining tasks: %d",
                    num_tasks,
                )
        ...
        if socks.get(tcdec_receiver) == zmq.POLLIN:
            ...
            if result_message == ZMQ_RESULT_SKIPPING:
                num_tasks -= 1
                log.debug(
                    "Skipped post-computation, remaining tasks: %d", num_tasks
                )
