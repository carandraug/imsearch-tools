...
class GoogleWebSearch(requests.Session, SearchClient):
    ...
    def __init__(self, async_query=True, timeout=5.0, **kwargs):
        ...
        self._supported_sizes_map = {"small": "s", "medium": "m", "large": "l"}
    ...
    def _fetch_results_from_offset(
        ...
        try:
            ...
            aux_params[
                "ijn"
            ] = page_idx  # ijn is the AJAX page index (0 = first page)
            ...
            resp = self.get(
                GOOGLE_WEB_ENTRY + GOOGLE_WEB_FUNC,
                params=aux_params,
                headers=headers,
                cookies={"CONSENT": "YES+"},
            )
    ...
    def query(self, query, size="medium", style="photo", num_results=100):
        ...
        aux_params[
            "ijn"
        ] = 0  # causes AJAX request contents only to be returned
        ...
        results = self._fetch_results(
            query, num_results, aux_params=aux_params, headers=headers
        )
