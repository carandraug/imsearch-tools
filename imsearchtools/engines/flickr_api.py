...
class FlickrAPISearch(requests.Session, SearchClient):
    ...
    def __init__(self, async_query=True, timeout=5.0, **kwargs):
        ...
        self._supported_sizes_map = {"small": "t", "medium": "n", "large": "c"}
    ...
    def _fetch_results_from_offset(
        ...
        try:
            ...
            if num_results > self._results_per_req:
                raise ValueError(
                    "num_results should be (%d) <= self._results_per_req (%d)"
                    % num_results,
                    self._results_per_req,
                )
            if result_offset % float(self._results_per_req):
                raise ValueError(
                    "Offset for Flickr API must be a multiple of self._results_per_req (%d)"
                    % self._results_per_req
                )
            ...
            resp = self.get(
                full_url
            )  # This only works with the full URL. It no longer works specifying the parameters in a separate variable.
    ...
    def query(self, query, size="medium", style="photo", num_results=100):
        ...
        aux_params = {
            ...
            "sort": "relevance",
            "content_type": 1,
        }  # just photos, 'style' parameter would be ignored
